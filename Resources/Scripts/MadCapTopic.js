...
(function () {
	...
	function setMediumChangeEvents() {
		...
		var mediums = _HelpSystem.Breakpoints.mediums;

		var $window = $(window);
		var prevWidth = $window.width();
		var currWidth = prevWidth;

		$window.on('resize', function () {
			currWidth = $window.width();
			for (var medium in mediums) {
				var breakpoint = mediums[medium];
				// if you pass a breakpoint set off a medium change event
				// use event.detail.medium to get medium name
				if (currWidth > breakpoint && breakpoint > prevWidth || currWidth < breakpoint && breakpoint < prevWidth) {
					var eventObject = {
						bubbles: false,
						detail: {
							medium: _HelpSystem.getLayout(currWidth)
						}
					};
	...
	function modifyHeight($heightContainer, isFluid, isTopNav, isFooter) {
		...
		if (isFluid) {
			margin = $heightContainer.offset().top + margin;
			$heightContainer.css('min-height', $(window).height() - margin);
		} else if ($heightContainer.is('body')) {
			$heightContainer.css('min-height', $(window).height() - margin);
		}
